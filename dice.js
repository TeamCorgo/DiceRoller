...
(function(dice) {
    ...
    this.create_dice_materials = function(face_labels, size, margin, type) {
        function create_text_texture(text, color, back_color, type) {
            ...
            if(type == 'd10' || type == 'd12' || type == 'd20'){
              if (text == '6' || text == '9') {
                context.fillText('_', canvas.width / 2, canvas.height / 2);
              }
            }
        ...
        for (var i = 0; i < face_labels.length; ++i) {
			...
			var FaceTexture = create_text_texture(face_labels[i], this.label_color, this.dice_color, type);

			//If Emoji set custom options 
			if (type == '2'){
				materials.push( new THREE.MeshPhongMaterial($t.copyto(this.material_optionsEmoj,{ map: FaceTexture, bumpMap: mapHeight, bumpScale: 1.3, specularMap: FaceTexture })));
			} else {
				materials.push( new THREE.MeshPhongMaterial($t.copyto(this.material_options,{ map: FaceTexture, bumpMap: mapHeight, bumpScale: 1.3,  specularMap: FaceTexture })));
			}
	...
	var mapHeight = new THREE.TextureLoader().load(url.getAttribute('data-url'));
	...
	this.known_types = [ 'd6', 'd8', 'd10', 'd12', 'd20', 'd100', 'd2', 'd3' ];
    this.dice_face_range = { 'd6': [1 ,6], 'd8': [1, 8], 'd10': [0, 9], 'd12': [1, 12], 'd20': [1, 20], 'd100': [0, 9], 'd2': [1 ,6], 'd3': [1 ,6]};
	this.dice_mass = { 'd6': 300, 'd8': 340, 'd10': 350, 'd12': 350, 'd20': 400, 'd100': 350 ,'d2': 300, 'd3': 300};
	this.dice_inertia = { 'd6': 13, 'd8': 10, 'd10': 9, 'd12': 8, 'd20': 6, 'd100': 9 ,'d2': 13, 'd3': 13};
    ...
    this.create_d6 = function() {
        ...
        this.dice_material = this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, 'd6');
    ...
    this.create_d8 = function() {
        ...
        this.dice_material = this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.2, 'd8');
    ...
    this.create_d10 = function() {
        ...
        this.dice_material = this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, 'd10');
    ...
    this.create_d12 = function() {
        ...
        this.dice_material = this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, 'd12');
    ...
    this.create_d20 = function() {
        ...
        this.dice_material = this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, 'd20');
        return new THREE.Mesh(this.d20_geometry, this.dice_material);
    }

    this.create_d100 = function() {
        if (!this.d10_geometry) this.d10_geometry = this.create_d10_geometry(this.scale * 0.9);
        this.d100_material = this.create_dice_materials(this.standart_d100_dice_face_labels, this.scale / 2, 1.5, 'd100');
        return new THREE.Mesh(this.d10_geometry, this.d100_material);
    }
	
    this.create_d2 = function() {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.9);
        this.dice_material = this.create_dice_materials([' ', 'X', 'Yes', 'No', 'Yes', 'No', 'No', 'Yes'], this.scale / 2, 1.9, 'd2');
		return new THREE.Mesh(this.d6_geometry, this.dice_material);
    }
	
    this.create_d3 = function() {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.9);
        this.dice_material = this.create_dice_materials([' ', 'X', '😎', '❤️', '🤣', '😍', '💩', '🍆'], this.scale / 2, .9, 2, 'd3');
		return new THREE.Mesh(this.d6_geometry, this.dice_material);
    }	
    ...
    this.dice_box.prototype.reinit = function(container, dimentions) {
		...
		//this.camera.rotation.z = -3.14; //Rotate Camera 180
        ...
        //this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), new THREE.MeshPhongMaterial({ color: that.desk_color }));
				this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), new CANNON.Material());	
    ...
    function make_random_vector(vector) {
        ...
        var vec = {
            ...
            y: vector.x * Math.sin(random_angle) + vector.y * Math.cos(random_angle) + 3
        };
    ...
    this.dice_box.prototype.generate_vectors = function(notation, vector, boost) {
        var vectors = [];
        boost = boost + 1000; //Speed up dice throw
    ...
    this.dice_box.prototype.check_if_throw_finished = function() {
        var res = true;
        var e = 1;
        if (this.iteration < 100 / that.frame_rate) {
    ...
    this.dice_box.prototype.__animate = function(threadid) {
        ...
        var time_diff = (time - this.last_time) / 2500;	//how fast the simulation is: default 1000
        if (time_diff > 1) time_diff = that.frame_rate;
        ++this.iteration;
    ...
    this.dice_box.prototype.__selector_animate = function(threadid) {
        var time = (new Date()).getTime();
        var time_diff = (time - this.last_time) / 700;
        if (time_diff > 3) time_diff = that.frame_rate;
        var angle_change = 0.3 * time_diff * Math.PI * Math.min(24000 + threadid, 6000) / 6000;
    ...
    this.dice_box.prototype.draw_selector = function() {
        ...
        for (var i = 0, pos = -3.5; i < that.known_types.length; ++i, ++pos) {
            var dice = $t.dice['create_' + that.known_types[i]]();
            dice.position.set(pos * step, 0, step * 0.5);
            dice.castShadow = true;
            dice.userData = that.known_types[i];
			dice.dice_type = that.known_types[i];
			dice.body = new CANNON.RigidBody(that.dice_mass[that.known_types[i]], dice.geometry.cannon_shape, this.dice_body_material);
			dice.body.position.set(pos.x, 10, pos.z);
			dice.body.quaternion.setFromAxisAngle(new CANNON.Vec3(100, 1000, 100), 10 * Math.PI * 2);
			this.dices.push(dice); 
			dice.rotation.z = (3.15); //Rotate the dice so they can be read easily.
			this.scene.add(dice);
			this.world.add(dice.body);
        }
    ...
    this.dice_box.prototype.bind_mouse = function(container, notation_getter, before_roll, after_roll) {
        ...
        $t.bind(container, ['mouseup', 'touchend'], function(ev) {
            if (box.rolling == false){location.reload();}
        });
    }
